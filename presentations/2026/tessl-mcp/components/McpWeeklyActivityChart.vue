...
const rows = (activityData.rows as Row[]).toSorted((a, b) => a.week_start.localeCompare(b.week_start))
...
const barWidth = Math.max(3, Math.min(9, plotWidth / Math.max(1, rows.length) * 0.34))
...
const initTicks = computed(() => [0, initMax * 0.25, initMax * 0.5, initMax * 0.75, initMax])
const toolTicks = computed(() => [0, toolMax * 0.5, toolMax])

const toolLine = computed(() => rows.map((row) => `${xForDate(row.week_start).toFixed(1)},${yForTool(row.tool_calls).toFixed(1)}`).join(' '))
...
<template>
  <section class="activity-chart">
    ...
    <svg class="activity-chart__svg" :viewBox="`0 0 ${width} ${height}`" role="img">
      <defs>
        ...
        <filter id="activity-line-glow" x="-40%" y="-40%" width="180%" height="180%">
      ...
      <rect :x="plot.left" :y="plot.top" :width="plotWidth" :height="plotHeight" class="activity-chart__plot-bg" />
      ...
      <polyline class="activity-chart__line" :points="toolLine" filter="url(#activity-line-glow)" />
      ...
      <g class="activity-chart__axis activity-chart__axis--right">
        <text :x="plot.left + plotWidth" :y="plot.top - 20" text-anchor="end">Tool calls</text>
        <text
      ...
      <g class="activity-chart__x-axis">
        ...
        <text :x="plot.left" :y="plot.top + plotHeight + 68" text-anchor="start">{{ first.week_start }}</text>
        <text :x="plot.left + plotWidth" :y="plot.top + plotHeight + 68" text-anchor="end">{{ latest.week_end }}</text>
      </g>

      <g class="activity-chart__legend">
        <rect :x="plot.left + plotWidth - 292" :y="plot.top + 14" width="12" height="38" rx="2" />
        <text :x="plot.left + plotWidth - 270" :y="plot.top + 38">initializations</text>
        <line :x1="plot.left + plotWidth - 132" :x2="plot.left + plotWidth - 96" :y1="plot.top + 33" :y2="plot.top + 33" />
        <text :x="plot.left + plotWidth - 86" :y="plot.top + 38">tool calls</text>
      </g>
    </svg>
  </section>
</template>
...
.activity-chart {
  ...
  background:
    radial-gradient(circle at 78% 12%, rgba(106, 163, 247, 0.14), transparent 28%),
    rgba(20, 22, 27, 0.74);
